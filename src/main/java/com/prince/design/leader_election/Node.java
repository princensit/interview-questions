...
public class Node extends UnicastRemoteObject implements ElectionNode {
    ...
    public static void main(String[] args) {
        String name = (args.length < 1 || args[0].equals("!")) ? "Node-" + System.currentTimeMillis() : args[0];
    ...
    /**
     * Starts the election. If the node has an ignore election counter it will decrement the counter and throws a
     * DeadNodeException. If the sender's name is lexicographically greater than or equal to this node's it will declare
     * it the new leader. Otherwise, it will start a new election with itself as the candidate.
     */
    @Override
    ...
    /**
     * The current leader receives messages through this method and returns a message back to the sender in order to let
     * the sender know the leader is still there and active.
     */
    @Override
    ...
    /**
     * Allows the ElectionDriver to change the name of the node and rebind it to the registry under the new name
     */
    @Override
